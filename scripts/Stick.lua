-- Wiggly Stick Controller - Fixed Z/X Buttons
...
mf.Size = UDim2.new(0, 280, 0, 380)
mf.Position = UDim2.new(0.5, -140, 0.5, -190)
...
tb.TextColor3 = Color3.fromRGB(255, 200, 100)
tb.TextSize = 12
...
local stat = label("Off", 0, 350, 280, 18)
    ...
    for _, p in pairs(WS:GetDescendants()) do
        if p:IsA("BasePart") and not p.Anchored and not p:IsDescendantOf(char) and p.Transparency < 0.5 then
            ...
            for _, c in pairs(p:GetChildren()) do
                if c:IsA("JointInstance") or c:IsA("Constraint") then free = false end
            end
            if free then
                table.insert(fp, {part = p, dist = (p.Position - root.Position).Magnitude})
            end
        end
    end
    
    if #fp == 0 then
        stat.Text = "No parts found"
        return
    end
    
    table.sort(fp, function(a, b) return a.dist < b.dist end)
    for i = 1, math.min(#mo, #fp) do
        table.insert(parts, fp[i].part)
    end
    
    cf = root.CFrame * CFrame.new(0, 5, -20)
    cycle = 0
    moving = false
    posStick()
    active = true
    stat.Text = "Stick: " .. #parts .. " parts"
    
    conn = RS.Heartbeat:Connect(function(dt)
        if not active then return end
        if kbd then
            root.CFrame = CFrame.new(cf.Position + cf:VectorToWorldSpace(Vector3.new(0, 10, 0)))
        end
        local mv = Vector3.new(dirs.Right - dirs.Left, dirs.Up - dirs.Down, dirs.Backward - dirs.Forward)
        moving = mv.Magnitude > 0
        if moving then
            cf += cf:VectorToWorldSpace(mv.Unit) * speed * dt
            cycle += dt * 3
        else
            cycle += dt * 1
        end
...
kbdBtn.MouseButton1Click:Connect(function()
    kbd = not kbd
    if kbd then
        kbdBtn.Text = "KEYBOARD: ON (WASD/QE/ZX)"
        ...
        velConn = RS.Heartbeat:Connect(function()
            if tick() >= et then velConn:Disconnect() return end
...
-- Direction buttons (W=Backward, S=Forward)
...
dirBtn("Q", 200, 100, Color3.fromRGB(100, 200, 255), "Up")
dirBtn("E", 200, 132, Color3.fromRGB(100, 200, 255), "Down")

-- Rotation buttons (Z=left, X=right) - FIXED
...
-- Speed
local sLabel = label("Speed: 30", 10, 170, 100, 18)
local sBox = Instance.new("TextBox", mf)
sBox.Size = UDim2.new(0, 50, 0, 20); sBox.Position = UDim2.new(0, 100, 0, 169)
...
btn("SET", 155, 168, 50, 22, Color3.fromRGB(100, 200, 100)).MouseButton1Click:Connect(function()
    local n = tonumber(sBox.Text)
    if n and n > 0 then speed = n; sLabel.Text = "Speed: " .. n end
end)

-- Wobbliness
local wLabel = label("Wobbliness: 5", 10, 198, 140, 18)
local wBox = Instance.new("TextBox", mf)
wBox.Size = UDim2.new(0, 50, 0, 20); wBox.Position = UDim2.new(0, 100, 0, 197)
...
btn("SET W", 155, 196, 50, 22, Color3.fromRGB(150, 100, 255)).MouseButton1Click:Connect(function()
    local n = tonumber(wBox.Text)
    if n then wobbliness = n; wLabel.Text = "Wobbliness: " .. n end
end)

-- Curliness
local curlLabel = label("Curliness: 0", 10, 226, 140, 18)
local curlBox = Instance.new("TextBox", mf)
curlBox.Size = UDim2.new(0, 50, 0, 20); curlBox.Position = UDim2.new(0, 100, 0, 225)
...
btn("SET C", 155, 224, 50, 22, Color3.fromRGB(255, 100, 200)).MouseButton1Click:Connect(function()
    local n = tonumber(curlBox.Text)
    if n then curliness = n; curlLabel.Text = "Curliness: " .. n end
end)

-- Length
local lLabel = label("Length: 400", 10, 254, 140, 18)
local lBox = Instance.new("TextBox", mf)
lBox.Size = UDim2.new(0, 50, 0, 20); lBox.Position = UDim2.new(0, 100, 0, 253)
lBox.BackgroundColor3 = Color3.fromRGB(40, 40, 40); lBox.TextColor3 = Color3.fromRGB(255, 255, 255)
lBox.Text = "400"; lBox.Font = Enum.Font.Gotham; lBox.TextSize = 10; lBox.BorderSizePixel = 0
Instance.new("UICorner", lBox).CornerRadius = UDim.new(0, 4)
btn("SET L", 155, 252, 50, 22, Color3.fromRGB(255, 200, 50)).MouseButton1Click:Connect(function()
    local n = tonumber(lBox.Text)
    if n and n > 0 then stickLength = n; lLabel.Text = "Length: " .. n end
end)

-- Presets
label("Presets:", 10, 282, 60, 18)
btn("Calm", 70, 280, 45, 22, Color3.fromRGB(50, 150, 200)).MouseButton1Click:Connect(function()
    wobbliness = 2; wBox.Text = "2"; wLabel.Text = "Wobbliness: 2"
    curliness = 0; curlBox.Text = "0"; curlLabel.Text = "Curliness: 0"
end)
btn("Normal", 120, 280, 50, 22, Color3.fromRGB(100, 200, 100)).MouseButton1Click:Connect(function()
    wobbliness = 5; wBox.Text = "5"; wLabel.Text = "Wobbliness: 5"
    curliness = 0; curlBox.Text = "0"; curlLabel.Text = "Curliness: 0"
end)
btn("Crazy", 175, 280, 45, 22, Color3.fromRGB(255, 150, 50)).MouseButton1Click:Connect(function()
    wobbliness = 12; wBox.Text = "12"; wLabel.Text = "Wobbliness: 12"
    curliness = 3; curlBox.Text = "3"; curlLabel.Text = "Curliness: 3"
end)
btn("INSANE", 225, 280, 50, 22, Color3.fromRGB(255, 50, 50)).MouseButton1Click:Connect(function()
    wobbliness = 20; wBox.Text = "20"; wLabel.Text = "Wobbliness: 20"
    curliness = 8; curlBox.Text = "8"; curlLabel.Text = "Curliness: 8"
end)

-- Keyboard binds
UIS.InputBegan:Connect(function(i, p)
    if p or not kbd then return end
    if i.KeyCode == Enum.KeyCode.W then dirs.Backward = 1
    elseif i.KeyCode == Enum.KeyCode.S then dirs.Forward = 1
    elseif i.KeyCode == Enum.KeyCode.A then dirs.Left = 1
    elseif i.KeyCode == Enum.KeyCode.D then dirs.Right = 1
    elseif i.KeyCode == Enum.KeyCode.Q then dirs.Up = 1
    elseif i.KeyCode == Enum.KeyCode.E then dirs.Down = 1
    elseif i.KeyCode == Enum.KeyCode.Z then rotL = true
    elseif i.KeyCode == Enum.KeyCode.X then rotR = true
    end
end)

UIS.InputEnded:Connect(function(i, p)
    if p or not kbd then return end
    if i.KeyCode == Enum.KeyCode.W then dirs.Backward = 0
    elseif i.KeyCode == Enum.KeyCode.S then dirs.Forward = 0
    elseif i.KeyCode == Enum.KeyCode.A then dirs.Left = 0
    elseif i.KeyCode == Enum.KeyCode.D then dirs.Right = 0
    elseif i.KeyCode == Enum.KeyCode.Q then dirs.Up = 0
    elseif i.KeyCode == Enum.KeyCode.E then dirs.Down = 0
    elseif i.KeyCode == Enum.KeyCode.Z then rotL = false
    elseif i.KeyCode == Enum.KeyCode.X then rotR = false
    end
end)
