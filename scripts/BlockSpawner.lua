...
local function findPlate()
    LPlate=nil Active=nil
    if WS:FindFirstChild("Plates")then
        for i,v in pairs(WS.Plates:GetChildren())do
            local o=v:FindFirstChild("Owner")
            if o and o.Value==LPlayer then
                LPlate=v:FindFirstChild("Plate")Active=v:FindFirstChild("ActiveParts")
            end
        end
    end
end
...
sg.Name="BlockSpawner"sg.ResetOnSpawn=false
